...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/03/18 13:41:44 by sgarba            #+#    #+#             */
/*   Updated: 2026/03/18 14:35:19 by sgarba           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_shift_buff(char *static_buff, size_t newline_position)
{
	char	*after_newline;
	size_t	i;
	size_t	after_newline_len;

	after_newline_len = ft_strlen(static_buff) - (newline_position + 1);
	if (!after_newline_len)
	{
		free(static_buff);
		return (NULL);
	}
	after_newline = malloc(after_newline_len + 1);
	...
	after_newline[after_newline_len] = '\0';
	free (static_buff);
